...
Post-hoc analysis using data from Exps 0-3. Produces:
  ...
  - Figure 8:  Temporal ordering — IPR regime shift and drift peak are concurrent
...
OUTPUT_DIR = "results/experiments/exp6_mechanistic/figures"
EXP2_DIR = "results/experiments/exp2_aggregation"
EXP3_DIR = "results/experiments/exp3_heterogeneity"
EXP3A_DIR = os.path.join(EXP3_DIR, "exp3a")
EXP3B_DIR = os.path.join(EXP3_DIR, "exp3b")
EXP4_DIR = "results/experiments/exp4_optimization"
...
def _ipr_changepoint(ipr_smooth, max_pts=5000):
    """Find the first regime shift in IPR using PELT (L2 cost).

    Subsamples to *max_pts* for speed, then maps the changepoint index
    back to the original resolution.
    """
    n = len(ipr_smooth)
    if n > max_pts:
        stride = n // max_pts
        signal = ipr_smooth[::stride].reshape(-1, 1)
    else:
        stride = 1
        signal = ipr_smooth.reshape(-1, 1)
    algo = rpt.Pelt(model="l2", min_size=max(5, 50 // stride)).fit(signal)
    pen = np.log(len(signal)) * np.var(signal)
    cps = algo.predict(pen=pen)
    cps = [c for c in cps if c < len(signal)]
    if cps:
        return min(cps[0] * stride, n - 1)
    return None


def _drift_peak_idx(drift_smooth):
    """Index of drift peak after initial warmup."""
    warmup = max(10, len(drift_smooth) // 20)
    return warmup + np.argmax(drift_smooth[warmup:])


def plot_figure8():
    """Figure 8: Temporal ordering — IPR regime shift and drift peak are concurrent.

    Both landmarks are threshold-free:
      - IPR onset: first changepoint detected by PELT (Killick et al. 2012)
        on smoothed IPR, using BIC-like penalty (log(n) * var).
      - Drift peak: argmax of smoothed client drift after warmup.

    Observation: The two events are statistically simultaneous (Wilcoxon
    p > 0.05), with strong correlation (r ~ 0.6). This is consistent with
    IPR rise and drift peak being two manifestations of the same phase
    transition rather than a causal sequence.
    """
    ...
    for run in grok_runs:
        h = run["h"]
        steps = run["steps"]
        ipr_s = smooth(np.array(h["ipr"]), window=100)
        drift_s = smooth(np.array(h["mean_client_drift"]), window=100)

        cp_idx = _ipr_changepoint(ipr_s)
        pk_idx = _drift_peak_idx(drift_s)

        if cp_idx is not None:
            t_ipr_cp.append(float(steps[cp_idx]))
            t_drift_pk.append(float(steps[pk_idx]))
            run_alphas.append(run["alpha"])

    t_ipr_cp = np.array(t_ipr_cp)
    t_drift_pk = np.array(t_drift_pk)
    lags = t_drift_pk - t_ipr_cp
    ...
    n_runs = len(lags)
    n_after = int(np.sum(lags > 0))
    _, w_pval = stats.wilcoxon(lags)
    med_lag = np.median(lags)
    r_val, _ = stats.pearsonr(t_ipr_cp, t_drift_pk)

    # ── Layout: 1×2 ──
    fig, (ax_scatter, ax_demo) = plt.subplots(
        1, 2, figsize=(13, 5.8),
        gridspec_kw={"width_ratios": [1, 1.15], "wspace": 0.35},
        constrained_layout=True)

    # ── (a) Scatter: T_IPR_cp vs T_drift_peak ──
    ax = ax_scatter
    ax.scatter(t_ipr_cp, t_drift_pk, c=colors, s=45, alpha=0.8,
               edgecolors="k", linewidths=0.3, zorder=3)
    lims = [0, max(t_ipr_cp.max(), t_drift_pk.max()) * 1.1]
    ax.plot(lims, lims, "k--", alpha=0.35, lw=1, label="Simultaneous")
    ax.fill_between(lims, lims, [lims[1]] * 2, alpha=0.06, color="#1565C0",
                    label="Drift peak after")
    ax.fill_between(lims, [0] * 2, lims, alpha=0.06, color="#C62828",
                    label="Drift peak before")
    ax.set_xlabel(r"$T_{\mathrm{IPR\ changepoint}}$ (steps)")
    ax.set_ylabel(r"$T_{\mathrm{drift\ peak}}$ (steps)")
    ax.set_xlim(lims)
    ax.set_ylim(lims)
    ax.set_aspect("equal")
    ax.legend(fontsize=8, loc="upper left")
    ...
    # Stats annotation
    ax.text(0.97, 0.03,
            f"$n = {n_runs}$,  $r = {r_val:.2f}$\n"
            f"Median lag $= {med_lag:+.0f}$ steps\n"
            f"Wilcoxon $p = {w_pval:.2f}$",
            transform=ax.transAxes, va="bottom", ha="right", fontsize=9,
            bbox=dict(boxstyle="round,pad=0.4", facecolor="white",
                      edgecolor="#999", alpha=0.95))

    # Alpha legend inside scatter
    from matplotlib.lines import Line2D
    alpha_handles = [Line2D([0], [0], marker="o", color="w", markersize=6,
                            markerfacecolor=alpha_cmap[a], markeredgecolor="k",
                            markeredgewidth=0.3, label=f"$\\alpha = {a}$")
                     for a in alphas_unique]
    ax.legend(handles=ax.get_legend_handles_labels()[1] and
              ax.get_legend().legend_handles + alpha_handles,
              labels=[t.get_text() for t in ax.get_legend().get_texts()]
              + [f"$\\alpha = {a}$" for a in alphas_unique],
              fontsize=7.5, loc="upper left", ncol=1,
              handletextpad=0.4, borderpad=0.5)

    ax.set_title(r"$\mathbf{(a)}$ IPR changepoint vs drift peak"
                 f"  ({n_after}/{n_runs} after)",
                 loc="left", fontsize=11)

    # ── (b) Single-run illustration ──
    ax = ax_demo
    demo_pattern = "*_a0.5_K10_*_dir1000.0_s42.json"
    demo_h = load_fl_history(EXP3A_DIR, demo_pattern)
    demo_steps = np.array(demo_h["total_steps"])
    demo_ipr_s = smooth(np.array(demo_h["ipr"]), window=100)
    demo_drift_s = smooth(np.array(demo_h["mean_client_drift"]), window=100)
    demo_acc = np.array(demo_h["test_acc"])
    demo_cp = _ipr_changepoint(demo_ipr_s)
    demo_pk = _drift_peak_idx(demo_drift_s)

    # Twin axes: left = drift, right = IPR, background = accuracy
    ax.fill_between(demo_steps, demo_acc, alpha=0.08, color="#888",
                    label="Test accuracy")
    ax.plot(demo_steps, demo_acc, color="#aaa", lw=0.8, alpha=0.5)

    ax2 = ax.twinx()

    ln1 = ax.plot(demo_steps, demo_drift_s, color="#1565C0", lw=1.8,
                  label="Client drift")[0]
    ln2 = ax2.plot(demo_steps, demo_ipr_s, color="#7B1FA2", lw=1.8,
                   label="IPR")[0]

    # Landmark lines
    cp_step = int(demo_steps[demo_cp])
    pk_step = int(demo_steps[demo_pk])
    ln3 = ax.axvline(cp_step, color="#D32F2F", lw=2.2, ls="-", alpha=0.85,
                     label=f"IPR changepoint ({cp_step})")
    ln4 = ax.axvline(pk_step, color="#2E7D32", lw=2.2, ls="--", alpha=0.85,
                     label=f"Drift peak ({pk_step})")

    # Shade the concurrent zone
    ax.axvspan(min(cp_step, pk_step), max(cp_step, pk_step),
               alpha=0.10, color="#FFD600", zorder=0)

    ax.set_xlabel("Training steps")
    ax.set_ylabel("Mean client drift", color="#1565C0")
    ax2.set_ylabel("IPR", color="#7B1FA2")
    ...
    ax.set_title(r"$\mathbf{(b)}$ Single run ($\alpha{=}0.5$, IID, $K{=}10$)",
                 loc="left", fontsize=11)
    ...
    plt.close()
    print(f"Saved: {path}")
    print(f"  {n_runs} grokking runs, {n_after}/{n_runs} drift peak after IPR cp")
    print(f"  Median lag: {med_lag:+.0f} steps, "
          f"Wilcoxon p = {w_pval:.2e}, r = {r_val:.3f}")
...
if __name__ == "__main__":
    ...
    print(f"\nSaved raw data: results/experiments/exp6_mechanistic/drift_vs_grokking.json")
